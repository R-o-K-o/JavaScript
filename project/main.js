...
//  так, что бы было видно их блоки (дать задний фон + margin. Иными словами - крайне четкая сетка)
    ...
    fetch(`https://jsonplaceholder.typicode.com/users`)
        ...
        .then(users => {
            ...
            usersCards.classList.add('users-cards');

            let title = document.createElement('h1');
            title.classList.add('title');
            title.innerText = 'user page';
            ...
            for (let user of users) {
                ...
                button.onclick = () => location.href = `user-details.html?user=${JSON.stringify('user', user)}`;

                userCard.appendChild(button);
                usersCards.appendChild(userCard);
            }
            container.append(title, usersCards);
        });
